...
class User:
    ...
    def help(self):
        ...
        message += "16. list_achievements\n"
        return (EXIT_SUCCESS, message)

    
    def add_language(self, name):
        try:
            self.language.append(Language(name, name + "_conf.xml"))
    ...
    def get_params(self):
        if (self.current_language is not None):
            return (EXIT_SUCCESS, self.current_language.get_params())
        else:
            return (NO_INSTANCED, "unspecified language")
