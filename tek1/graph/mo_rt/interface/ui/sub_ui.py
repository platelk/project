...
class uiAddObject(uiDialog):
    ...
    def init(self, obj):
        ...
        if (self.Obj != "camera" and self.Obj != "light"):
            ...
            self.Coor.setLayout(hLayout)
            self.GridLay.addWidget(self.Coor, 2, 1)

            self.Coor = QGroupBox("Effect", self)
            hLayout = QHBoxLayout(self)

            self.effect = effect(self.Parent)
        ...
        hLayout.addWidget(act)
        act = QPushButton("Prev", self)
        act.connect(act, SIGNAL("clicked()"), self.setPrev)
        hLayout.addWidget(act)
    ...
    def setPrev(self):
        import ui
        self.Run = ui.rayTracer()
        prev = "<window y=120 x=120>\n  <scene>\n"
        ...
        self.GridLay.addWidget(image, 1, 0)
        self.show()
