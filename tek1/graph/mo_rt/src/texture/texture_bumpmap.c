...
** Last update Wed May 30 17:28:07 2012 ronan guiho
*/
...
double		texture_bump_mapping_height(int x, int y, t_object *obj)
{
  ...
  if (obj->perlin != NULL && (obj->perlin->apply == APP_BUMP || \
			      obj->perlin->apply == APP_BUMP_TEXT))
    {
      ...
      color.red = (obj->perlin->begin->red * (1.0 - height) + obj->perlin->end->red * \
		   height);
      color.green = (obj->perlin->begin->green * (1.0 - height) + obj->perlin->end->green * \
		   height);
      color.blue = (obj->perlin->begin->blue * (1.0 - height) + obj->perlin->end->blue * \
		   height);
...
int		texture_bump_mapping(t_detail *detail, double *vector)
{
  ...
  utils[1] = (texture_bump_mapping_height(detail->object->texture_pos[X], \
	       ...
	       texture_bump_mapping_height(detail->object->texture_pos[X], \
					   detail->object->texture_pos[Y] - 1, \
					   detail->object));
  vector[X] = (1.0f - ((detail->object->perlin != NULL && (detail->object->perlin->apply == APP_BUMP ||
							   detail->object->perlin->apply == APP_BUMP_TEXT)) ? \
		       detail->object->perlin->relief : detail->object->image_bump->relief)) * vector[X] + \
    ((detail->object->perlin != NULL && (detail->object->perlin->apply == APP_BUMP || \
					 detail->object->perlin->apply == APP_BUMP_TEXT)) ? \
     detail->object->perlin->relief : detail->object->image_bump->relief) * utils[0];
  vector[Y] = (1.0f - ((detail->object->perlin != NULL && (detail->object->perlin->apply == APP_BUMP ||
							   detail->object->perlin->apply == APP_BUMP_TEXT)) ? \
		       detail->object->perlin->relief : detail->object->image_bump->relief)) * vector[Y] + \
    ((detail->object->perlin != NULL && (detail->object->perlin->apply == APP_BUMP || \
					 detail->object->perlin->apply == APP_BUMP_TEXT)) ? \
     detail->object->perlin->relief : detail->object->image_bump->relief) * utils[1];
  vector[Z] = (1.0f - ((detail->object->perlin != NULL && (detail->object->perlin->apply == APP_BUMP ||
							   detail->object->perlin->apply == APP_BUMP_TEXT)) ? \
		       detail->object->perlin->relief : detail->object->image_bump->relief)) * vector[Z] + \
    ((detail->object->perlin != NULL && (detail->object->perlin->apply == APP_BUMP || \
					 detail->object->perlin->apply == APP_BUMP_TEXT)) ? \
     detail->object->perlin->relief : detail->object->image_bump->relief) * 1.0;
  return (EXIT_SUCCESS);
}
