...
** Last update Mon May 28 22:01:36 2012 ronan guiho
*/
				     ...
				     double *old_pos, t_image *image)
{
  ...
  texture[X] = (int)(pos[X] * FREQUENCE_PLAN + \
		     ((image != NULL) ? image->size[WIDTH] : SIZE_X) / 2.0);
  texture[Y] = (int)(pos[Y] * FREQUENCE_PLAN + \
		     ((image != NULL) ? image->size[HEIGHT] : SIZE_Y) / 2.0);
  while (texture[X] < 0)
    texture[X] = texture[X] + (100 * ((image != NULL) ? image->size[WIDTH] : SIZE_X));
  while (texture[Y] < 0)
    texture[Y] = texture[Y] + (100 * ((image != NULL) ? image->size[HEIGHT] : SIZE_Y));
