...
** Last update Mon May 28 18:36:08 2012 ronan guiho
*/
				     ...
				     double *old_pos, t_image *image)
{
  ...
  utils[0] = (1.0 - (atan2(pos[Z], pos[X]) + M_PI) / TWO_M_PI) * image->size[WIDTH];
  utils[2] = sqrt(pos[X] * pos[X] + pos[Z] * pos[Z]);
  pos[X] = utils[2] - radius[MAX];
  utils[1] = ((atan2(pos[Y], pos[X]) + M_PI) / TWO_M_PI) * image->size[HEIGHT];
