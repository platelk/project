...
** Last update Mon May 28 00:29:05 2012 alexis leprovost
*/
			     ...
			     double *vector, double *indice)
{
  ...
  indice[A] = (CARRE(u[X]) / CARRE(cst[A])) + (CARRE(u[Y]) / CARRE(cst[B])) - (CARRE(u[Z]) / CARRE(cst[C]));
  indice[B] = 2.0F * (((pos[X] * u[X]) / CARRE(cst[A])) + ((pos[Y] * u[Y]) / CARRE(cst[B])) - ((pos[Z] * u[Z]) / CARRE(cst[C])));
  indice[C] = (CARRE(pos[X]) / CARRE(cst[A])) + (CARRE(pos[Y]) / CARRE(cst[B])) - (CARRE(pos[Z]) / CARRE(cst[C])) - 1.0F;
  indice[DELTA] = CARRE(indice[B]) - (4.0F * indice[A] * indice[C]);
  return (EXIT_SUCCESS);
}

int
inter_hyperboloide(t_camera *camera, t_object *object, t_detail *detail, double *vector)
{
