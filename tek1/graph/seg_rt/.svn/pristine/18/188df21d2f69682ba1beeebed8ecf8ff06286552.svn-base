...
** Last update Mon May 28 00:28:29 2012 alexis leprovost
*/
...
static int	inter_indice_cubetroue(t_camera *camera, t_object *object,
				       double *vector, double *indice)
{
  ...
  indice[B] = 4.0 * (CUBE(u[X]) * pos[X] + CUBE(u[Y]) * pos[Y] + CUBE(u[Z]) * pos[Z]);
  indice[C] = 6.0 * (CARRE(u[X]) * CARRE(pos[X]) + CARRE(u[Y]) * CARRE(pos[Y]) + CARRE(u[Z]) * CARRE(pos[Z])) - \
    5.0 * (CARRE(u[X]) + CARRE(u[Y]) + CARRE(u[Z]));
  indice[D] = 4.0 * (CUBE(pos[X]) * u[X] + CUBE(pos[Y]) * u[Y] + CUBE(pos[Z]) * u[Z]) - 10.0 * \
    (u[X] * pos[X] + u[Y] * pos[Y] + u[Z] * pos[Z]);
  indice[E] = QUATRO(pos[X]) + QUATRO(pos[Y]) + QUATRO(pos[Z]) - 5.0 * (CARRE(pos[X]) + CARRE(pos[Y]) + CARRE(pos[Z]));
  indice[E] += size;
  return (EXIT_SUCCESS);
}
