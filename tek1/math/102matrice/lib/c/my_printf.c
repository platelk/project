...
** Last update Sun Dec  4 08:14:51 2011 kevin platel
*/
...
#define IS_ALPHA (my_is_alpha(format[*i]) == 1)
#define IS_NOT_IN_OPT  (find_intab(format[*i + 1]) == -1)
...
int	opt(const char *format, int *i, int *cont, va_list *ap)
{
  ...
  j = *i;
  tab = init_tab_func();
  while (format[(*i) + 1] != '\0' && IS_NOT_IN_OPT && IS_ALPHA)
    (*i)++;
      if (format[(*i) + 1] == '\0' || IS_NOT_IN_OPT)
	{
	  my_putchar('%');
	  (*i) = j;
	  return (-1);
