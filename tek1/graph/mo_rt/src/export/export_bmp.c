...
** Last update Fri May 25 10:00:41 2012 ronan guiho
*/
...
int		write_headerbmp(t_image *img, int fd)
{
  ...
  write(fd, &header, sizeof(t_headerbmp));
  return (EXIT_SUCCESS);
}
...
int		write_pixelbmp(int fd, int R, int G, int B)
{
  ...
  colorbmp.B = B;
  colorbmp.A = 0;
  write(fd, &colorbmp, sizeof(t_colorbmp));
  return (EXIT_SUCCESS);
}

int		export_bmp(t_global *global, char *name)
{
  int		fd;
  int		x;
  int		y;
  ...
  i = 0;
  if ((img = export_image(global)) == NULL)
    return (EXIT_FAILURE);
