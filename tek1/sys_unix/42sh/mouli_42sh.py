...
class   Testing:
    ...
    def addListFail(self, nb):
        Testing.NbListFail.append(nb)

    def addListSuccess(self, nb):
        Testing.NbListSuccess.append(nb)

    def TestFail(self):
        return (Testing.NbListFail)

    def TestSuccess(self):
        return (Testing.NbListSuccess)
    ...
    def runTest(self, nb):
        ...
        if ("Segmentation" in aff) or ("segmentation" in aff):
            color(red)
            stdout.write(" Segfault ! ")
            color(white)
            self.NbTestSeg.append(nb)
...
def usage():
    ...
    print "-p : print percent of success test"
    print "-o TEST: launch mouli with this test"
...
try:
        opts, args = getopt(argv[1:], "us:mt:vl:e:rf:aib:c:o:p", ["usage", "start=", "manuel", "test=",
        "verb", "list=", "end=", "recap", "file=", "all", "i", "bin=", "check=", "one=", "percent"])
...
if (one != None):
    ...
    while (test != None):
        ...
        nb += 1
        unit.addListSuccess(unit.TestListSuccess())
        unit.addListFail(unit.TestListFail())
        test = list_of_test(nb)

if (recap == True):
    print ("List of result with [OK] return (" + str(unit.NbTestSuccess) + ") :")
    color(green)
    print unit.TestListSuccess()
    color(white)
    print ("List of result with [Error] return (" + str(unit.NbTestFail) + ") :")
    color(red)
    print unit.TestListFail()
    color(white)
    print ("List of result with [Segfault] return (" + str(len(unit.NbTestSeg)) + ") :")
    color(red)
    print unit.NbTestSeg
    color(white)
...
if (per == True):
    stdout.write("\nYou have success ")
    p = (((float) (unit.NbTestSuccess) / (float) (unit.NbTotTest)) * 100.0)
    if (p < 60.0):
        color(red)
    if (p > 60.0):
        color(yellow)
