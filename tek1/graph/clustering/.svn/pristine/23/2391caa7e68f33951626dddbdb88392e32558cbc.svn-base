...
** Last update Sat May  5 23:23:29 2012 ronan guiho
*/
  ...
  while (tmp_client)
      ...
      while (tmp_plage)
	{
	  ...
	  if ((send(tmp_client->sock, trame, my_strlen(trame), 0)) <= 0)
	    return (EXIT_FAILURE);
	  printf("[*] Raytracer: Clustering: Send [%s] to %s:%d.\n",
		 trame, inet_ntoa(tmp_client->csin->sin_addr), \
		 htons(tmp_client->csin->sin_port));
