...
** Last update Tue May 29 18:56:09 2012 zoe phalippou
*/
...
int
inter_cylindre(t_camera *camera, t_object *object, t_detail *detail, double *vector)
{
  ...
  if (indice[DELTA] >= 0.000)
    {
      if (calc_k(indice, k))
	return (EXIT_FAILURE);
      calc_limit(object->limit, camera->position, vector, &k[0]);
      calc_limit(object->limit, camera->position, vector, &k[1]);
      if (object->inhibitor)
